...
if busca <> "" then
	...
	Where_Busca = Where_Busca & " and Tab."& campo_tabela &"_Outros like '%" & busca & "%'"
end if

strURL = "listar.asp?w=0"&url_busca&"&tab="&tabela
...
Conexao.Open Application("cnn")
	...
	SQL_Listar = 	"Select " &_
					...
					"	,Tab.ID_Relacionamento_" & campo_tabela & " " &_
					"	,Tab.ID_Empresa " &_
					"	,Tab.ID_" & campo_tabela & " " &_
					"	,Tab." & campo_tabela & "_Outros " &_
					"	,Tab.Data_Cadastro " &_
					"From Relacionamento_" & campo_tabela & " as Tab "&_
					"Inner Join Empresas as E ON Tab.ID_Empresa = E.ID_Empresa "&_
					"Where Tab." & campo_tabela & "_Outros <> '' "&_
					Where_Busca &_
					"Order by Tab." & campo_tabela & "_Outros ASC, E.Razao_Social ASC"
  ...
  <tr>
    <td align="center" bgcolor="#FFFFFF"><table width="900" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td valign="top" style="font-family:Arial, Helvetica, sans-serif; font-size:18px; font-weight:bold;" align="center"><span style="color: #B01D22"><%=campo_tabela%> Outros</span></td>
	...
	<!--BUSCA-->
      <table width="900" border="0" cellpadding="0" cellspacing="0">
        ...
        <tr>
          ...
          <td>
          	<form action="/admin/outros/listar.asp?tab=<%=tabela%>" method="post" name="busca">
                <span class="titulo_noticias_home" style="width: 500px; float: left;">
                    ...
                    <%=campo_tabela%>: <input type="text" name="busca" id="busca" class="admin_txtfield_login">
		...
		%>
            <table width="100%" border="0" cellpadding="2" cellspacing="1" class="fs11px t_arial">
              <tr>
                ...
                <td width="300" class="borda_dir linha_16px" align="center"><b>Empresa</b></td>
                <td width="300" class="borda_dir linha_16px" align="center"><b>Ramos Outros</b></td>
			...
			SQL_Cadastro = 	"SELECT "&_
							...
							"	,ID_Empresa "&_
						  	...
						  	"Where ID_Empresa = '" & RS_Listar("ID_Empresa") & "'"
			...
			ID_Registro = "ID_Relacionamento_"&campo_tabela
	            %>
              <tr bgcolor="#FFFFFF" 
                onMouseOver="$(this).attr('bgcolor','#FFFF00'); " 
                onMouseOut="$(this).attr('bgcolor','#FFFFFF'); "  >
                <td class="borda_dir linha_16px cursor" onClick="document.location = 'editar.asp?tab=<%=tabela%>&id=<%=Rs_listar(ID_Registro)%>';" align="left"><%=RS_Listar(ID_Registro)%></td>
                <td class="borda_dir linha_16px cursor" onClick="document.location = 'editar.asp?tab=<%=tabela%>&id=<%=Rs_listar(ID_Registro)%>';" align="left"><%=RS_Listar("Empresa")%></td>
                <td class="borda_dir linha_16px cursor" onClick="document.location = 'editar.asp?tab=<%=tabela%>&id=<%=Rs_listar(ID_Registro)%>';" align="left"><%=RS_Listar(campo_tabela & "_Outros")%></td>
                <td class="borda_dir linha_16px cursor" onClick="document.location = 'editar.asp?tab=<%=tabela%>&id=<%=Rs_listar(ID_Registro)%>';" align="left"><%=FormatDateTIme(RS_Listar("Data_Cadastro"),2)%></td>
                <td class="borda_dir linha_16px cursor" align="center">
                	<a href="/admin/relatorios/relatorio_precredenciados_detalhes.asp?id=<%=ID_Cadastro%>">
                    	<img src="/admin/images/ico_preview_20_b.gif" width="15" height="15" alt="Cadastro da Empresa" title="Cadastro da Empresa" border="0"> 
                    </a>
                    <a href="editar.asp?tab=<%=tabela%>&id=<%=RS_Listar(ID_Registro)%>">
                		<img src="/admin/images/ico_pg_prox.gif" width="15" height="15" alt="Atualizar Produto" title="Atualizar Produto" border="0">
                    </a>
                </td>
              </tr>
              <%
