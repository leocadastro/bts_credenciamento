      ...
      <div id="aviso" style="background-color:#FFFF00; width:100%; text-align:center;" class="t_arial fs11px bold c_preto"> <img src="/admin/images/ico_aviso.gif" alt="Aviso" width="20" height="20" hspace="2" vspace="4" align="absmiddle" title="Aviso"> <span id="aviso_conteudo">Aviso Processando, por favor não interrompa.</span></div>
		...
		If not RS_1_RelacionamentoCadastros.BOF or not RS_1_RelacionamentoCadastros.EOF Then
			...
			While not RS_1_RelacionamentoCadastros.EOF
				...
				'==================================================
				' Nova Implementacao - 05/05/2014 - Leandro Santiago
				' INICIO - Codigo do convite
				'==================================================
				convite		= RS_1_RelacionamentoCadastros.Fields("CodConvite").Value
				'==================================================
				' FIM - Codigo do convite
				'==================================================

				IDs(i) = Array(id, idioma, tipo, visitante, empresa, formulario, data, convite)
...
'			StartTime = Timer()
		...
		oFiletxt.WriteLine("<td rowspan='3'>Cod. Convite</td>")
...
'response.write("<hr><b>SQL_2_Visitantes</b><br>" & SQL_2_Visitantes & "<br>")\
			...
			If IDs(x)(5) <> "4" AND Len(Trim(IDs(x)(4))) > 0 Then
				...
				SQL_6_Empresa = 	"Select " &_
									...
									"Where  " &_
									"	ID_Empresa = " & IDs(x)(4)
				'Response.Write("Empresa: " & SQL_6_Empresa & "<br>")
				...
				oFiletxt.WriteLine("<tr>")
					...
					If qtde_ramos > 0 Then
						...
						while w < Ubound(Ramos_Atividades)
							...
							sRamAtiv0=sRamAtiv0 & trocar(Ucase( RemoverAcentuacao(arrRamAtiv(0)) )) & ";"
							If Len(Trim(arrRamAtiv(1))) > 0 Then
								sRamAtiv1=sRamAtiv1 & trocar(Ucase( RemoverAcentuacao(arrRamAtiv(1)) )) & ";"
							End If
							w=w+1
						wend
						oFiletxt.WriteLine("<td nowrap>" & sRamAtiv0 & "</td>")
						oFiletxt.WriteLine("<td nowrap>" & sRamAtiv1 & "</td>")

					Else
						oFiletxt.WriteLine("<td nowrap> - </td>")
						oFiletxt.WriteLine("<td nowrap> - </td>")
					End If

					'==================================================
					' Nova Implementacao - 28/05/2014 - Leandro Santiago | HD 8615
					' INICIO - Correcao do retorno de Ramos e atividades repetindo nos registros
					'==================================================
					Redim Ramos_Atividades(0)
					'==================================================
					' FIM - Correcao do retorno de Ramos e atividades repetindo nos registros
					'==================================================
					...
					oFiletxt.WriteLine("<td nowrap>'" & trocar(Ucase( convite )) & "</td>")
					...
					For id_perg = Lbound(Array_ID_Perguntas)+1 To Ubound(Array_ID_Perguntas)
						...
						SQL_Listar_Resposta_Pergunta_por_Item = 	"Select Distinct " &_
																	...
																	"	INNER JOIN Relacionamento_Edicoes_Pergunta as REP " &_ 
															        "   	ON REP.ID_Pergunta = RP.ID_Perguntas " &_ 
															        "   	AND REP.ID_Edicao = " & id_edicao & " " &_ 
															        "   	AND REP.ID_Formulario = " & IDs(x)(5)  & " " &_
																	"Where  " &_
						...
						If not RS_Listar_Resposta_Pergunta_por_Item.BOF or not RS_Listar_Resposta_Pergunta_por_Item.EOF Then
							While not RS_Listar_Resposta_Pergunta_por_Item.EOF
								...
								vRespostas = split(resposta_a_esta_pergunta,";")

								For vr = lBound(vRespostas) to uBound(vRespostas)
									If Trim(RS_Listar_Resposta_Pergunta_por_Item.Fields("Resposta").Value)  = Trim(VRespostas(vr)) Then
										VR_Resposta = 1
										Exit For
									End If
								Next

								If VR_Resposta = 0 Then
