...
function EnviarConfirmacao() {
  ...
  var answer = confirm("Tem certeza que Reenviar o E-mail de confirmação?")  
    if (answer) {
      alert("sim")
        window.location = "processar.asp?id_edicao=<%=id_edicao%>&id_idioma=<%=id_idioma%>&ID_Formulario=<%=tipo_formulario%>&email=<%=Lcase(Email)%>&id=<%=ID%>&cpf=<%=CPF%>&nome=<%=Nome_Completo%>&cargo=<%=ID_Cargo%>&depto=<%=ID_Depto%>&cnpj=<%=CNPJ%>&razao=<%=Razao%>"
