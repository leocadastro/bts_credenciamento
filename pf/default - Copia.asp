...
%>
    ...
    <input type="hidden" id="id_visitante" 	name="id_visitante" 	value="" >
            ...
            <fieldset id="grupo3">
                ...
                <!-- O que voce procura na? 
				<label style="width:700px">				
				<%=textos_array(16)(2)%> <strong><%=Evento%> </strong>?</label>
                	<div id="divInteresse">
                        <ul>
                            <%
            ...
            <% 
	            ...
	            <% 								
	       		...
	       		%>               
									<%=IniLI%>
                                    <input type="<%=TipoCampo%>" name="frmPergunta_<%=qtde_perguntas%>" id="frmPergunta_<%=qtde_perguntas%>" value="<%=ValorCampo%>" <%=FormCampo%> />
                                    <span class="cursor" onclick="$('input[name=frmPergunta_<%=qtde_perguntas%>]')[<%=z%>].click();">&nbsp;&nbsp;<%=RS_Opcoes("Opcao")%></span>
									<%=FimLI%>
	            <%
										z = z + 1
										RS_Opcoes.MoveNext()
            ...
            <!-- FIM IMPLANTAÇÃO DOUGLAS 2014 -->
            
            
            
            <%
			'If Session("cliente_edicao") = 22 Then
            '<br/><br/>
            'Quer adquirir seu <strong>Ingresso</strong> antecipadamente? <strong>Clique</strong> no banner que estará na próxima página de <strong>Confirmação de Cadastro</strong>
            'End If
			%>
