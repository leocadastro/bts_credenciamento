...
'=======================================================================
	...
	If origem_cnpj = "novo" AND Len(id_empresa) > 0 Then
		...
		If not RS_Verificar_Empresa.BOF or not RS_Verificar_Empresa.EOF Then
			SQL_Atualizar_Empresa = "Update Empresas " &_
									...
									"	Nome_Fantasia 			= Upper(dbo.sp_rm_accent_pt_latin1('" & Fantasia 	& "')), " &_
									"	Principal_Produto 		= Upper(dbo.sp_rm_accent_pt_latin1('" & PriProdut 	& "')), " &_
...
'=======================================================================
	...
	If (origem_cnpj = "" OR origem_cnpj = "old") AND Len(Trim(id_empresa)) = 0 Then
		...
		SQL_Cad_End_Empresa = 	"INSERT INTO Relacionamento_Enderecos " &_
								...
								"	," & Pais & " " &_
								"	);"
		 response.write("<hr><b>SQL_Cad_End_Visitante</b><br>" & SQL_Cad_End_Empresa & "<hr>")
		' Executando Gravação
