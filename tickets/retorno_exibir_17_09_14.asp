...
If Not RS_Consulta_Pedido. Eof Then
	...
	<% End IF
		...
		'====================================
		Retorno = Split(Retorno_Pedido,"<BR>")
		
		Aprovacao 	= Split(Retorno(0),"|")
		Transacao 	= Split(Retorno(1),"|")
		Sequencial	= Split(Retorno(2),"|")
		Codigo		= Split(Retorno(3),"|")
		Valor		= Split(Retorno(4),"|")
		Resultado	= Split(Retorno(5),"|")
		AVS			= Split(Retorno(6),"|")
		Mascara		= Split(Retorno(7),"|")
		Pedido		= Split(Retorno(8),"|")
		Bandeira	= Split(Retorno(9),"|")
		Mes			= Split(Retorno(10),"|")
		FormaPagto	= Split(Retorno(11),"|")
		Parcelas	= Split(Retorno(12),"|")
		Ano			= Split(Retorno(13),"|")
		Numcontrole	= Split(Retorno(14),"|")
							...
							<legend>Detalhes do Pagamento:
									...
									<div style="font-weight: 100; width: 575px; float: left; font-size: 14px; font-size: 14px; padding: 5px 0 5px 5px;">
										...
										<script language="javascript">
                                            $(document).ready(function(){
                                                $.ajax({
                                                    url: "/tickets/retorno_xml.asp?transacao=<%=Transacao(1)%>",
                                                    success: function(data){
                                                        
                                                        Resposta = data.split(' ');
                                                        //alert(Resposta[0])
                                                        if(Resposta[0]=='Erro'){
                                                            jAlert("Erro na Transa&ccedil;&atilde;o","Erro");
                                                        }
                                            ...
                                            <div style="float:right; padding-right:5px;">
                                                <a href="recuperar_confirmacao_pagamento.asp?pedido=<%=Pedido(1)%>" target="_blank">
                                                    <img src="/img/geral/icones/msg_print.png" width="203" border="0" title="Imprimir Confirma&ccedil;&atilde;o de Pagamento" alt="Imprimir Confirma&ccedil;&atilde;o de Pagamento" style="cursor: pointer;"/>
                                                </a>
                                            </div>
											<br><br>
                                            <div style="padding: 5px 0; width: 180px; float: left">Pagamento:</div> 								<div style="padding: 5px 0; font-weight: 900">Aprovado</div>
                                            <div style="padding: 5px 0; width: 180px; float: left">Numero do Pedido:</div>							<div style="padding: 5px 0; font-weight: 900"><%=Pedido(1)%></div>
                                            <div style="padding: 5px 0; width: 180px; float: left">Transa&ccedil;&atilde;o:</div> 					<div style="padding: 5px 0; font-weight: 900"><%=Transacao(1)%></div>
                                            <div style="padding: 5px 0; width: 180px; float: left">C&oacute;d. da Autoriza&ccedil;&atilde;o:</div> 	<div style="padding: 5px 0; font-weight: 900"><%=Codigo(1)%></div>
                                            <div style="padding: 5px 0; width: 180px; float: left">Valor Pago:</div> 								<div style="padding: 5px 0; font-weight: 900"><%If Cint(Idioma) = 1 then Response.Write("R$") Else Response.Write("$")%>&nbsp;<%=FormatNumber(Valor(1),2)%></div>
                                            <div style="padding: 5px 0; width: 180px; float: left">Forma de Pagamento:</div> 						<div style="padding: 5px 0; font-weight: 900"><%=Bandeira(1)%></div>
                                            <div style="padding: 5px 0; width: 180px; float: left">Cart&atilde;o Utilizado:</div> 					<div style="padding: 5px 0; font-weight: 900"><%=Mascara(1)%></div><br>
										<%
										'Session("cliente_enviar_email") = 0
										
										If Session("cliente_enviar_email") = "" Or Session("cliente_enviar_email") = 0 Then
											
											Enviar_Email_Senha Session("cliente_edicao"), Session("cliente_idioma"), "", "", "", "", "", "Enviar_Ticket", Pedido(1), ID_Visitante
										...
										%>
                                            ...
                                            <div style="width: 100%; float: left"><%=URLDecode(Resultado(1))%></div><br>
                                            <div style="width: 100%; float: left"><em><strong>*Mensagem de erro recebida da sua operadora de cart&atilde;o.</strong></em></div><br><br>
                                        <%End If%>
									</div>
                                    <% 
									' Se pedido APROVADO
									If Cstr(Aprovacao(1)) = "True" Then 
										%>
										<br /><div style="font-size:16px; color:#1F497D; font-weight:bold; font-family:'Calibri','sans-serif'; text-align:center;">Caro Visitante, obrigado por ter efetuado sua compra! Retire seu ingresso nos guich&ecirc;s do PAVILH&Atilde;O AZUL do Expo Center Norte.</div><br /><br />
									...
									If Cstr(Aprovacao(1)) = "False" then
										%>
									...
									Else
										%>
											<img src="/img/geral/ico_ajax-loader.gif" id="loader_1" style="float: right">
											<a href="#_" onclick="link('status.asp');"><div id="loader_2" class="bt_meus_pedidos" style="float: right; display: none">Finalizar</div></a>
										<%
									End If
									%>
									</div>
