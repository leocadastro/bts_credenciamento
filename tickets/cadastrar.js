...
function atualizar_credencial (id, nome, email, curso, id_tipo) {
	$('#editar').fadeIn();
	$('#frmID').val(id);
	$('#frmTipo2').val(id_tipo);
	$('#frmEditarNome').val(nome);
	$('#frmEditarEmail').val(email);
...
//	$.getJSON('credencial_cadastrar.asp?id_tipo=' + $('#frmTipo').val() + '&nome=' + $('#frmNome').val() + '&curso=' + $('#frmCurso').val(), function(data, textStatus) {
	$.post(
		'credencial_cadastrar.asp',
	  	{ id_tipo : $('#frmTipo').val(), nome : $('#frmNome').val(), email : $('#frmEmail').val(), curso: $('#frmCurso').val() },
...
function atualizar () {
	$.getJSON('credencial_atualizar.asp?id=' + $('#frmID').val() + '&id_tipo=' + $('#frmTipo2').val() + '&nome=' + $('#frmEditarNome').val() + '&email=' + $('#frmEditarEmail').val() + '&curso=' + $('#frmEditarCurso').val(), function(data, textStatus) {
		$("#loading").fadeOut().height(452).css( {"z-index": 1}).hide();
		// verificar erro de retorno
		if (textStatus == 'success') {
			clearTimeout(timeout);
