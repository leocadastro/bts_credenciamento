...
<% End IF
	...
	' Select de s de Atividades 
	SQL_RamoAtividade = 	"SELECT " &_
							"	R.ID_Ramo_Atividade as Id, " &_
							"	R.Ramo_Atv_" & SgIdioma & " as Nome, " &_
							"	R.Complemento " &_
							"FROM Relacionamento_RamoeAtividade_Edicoes_V2 as RER " &_
							"Inner Join RamoeAtividade_V2  as R ON R.ID_Ramo_Atividade = RER.ID_Ramo_Atividade " &_
							"WHERE " &_
							"	R.Ativo = 1 " &_
							"	AND RER.ID_Edicao = " & ID_Edicao & " " &_
							"	AND R.Ramo_Atv_" & SgIdioma & " IS NOT NULL " &_
							"	AND R.ID_Ramo_Atividade <> -1 " &_
							"ORDER BY Nome " 
	'response.write(SQL_RamoAtividade)
	Set RS_RamoAtividade = Server.CreateObject("ADODB.Recordset")
	RS_RamoAtividade.CursorType = 0
	RS_RamoAtividade.LockType = 1
	RS_RamoAtividade.Open SQL_RamoAtividade, Conexao
	
	' Select de Funcionarios
	SQL_Funcionarios =	"SELECT " &_
						"	ID_Funcionarios_Qtde as Id, " &_
						"	Funcionarios_Qtde_" & SgIdioma & " as Func " &_
						"FROM Funcionarios_Qtde " &_
						"WHERE " &_
						"	Ativo = 1 " &_
						"ORDER BY ID_Funcionarios_Qtde " 
	Set RS_Funcionarios = Server.CreateObject("ADODB.Recordset")
	RS_Funcionarios.CursorType = 0
	RS_Funcionarios.LockType = 1
	RS_Funcionarios.Open SQL_Funcionarios, Conexao
	
	' Select de Interesses da Feira
	SQL_InteresseFeira ="SELECT " &_
						"	A.ID_AreaInteresse as Id, " &_
						"	A.AreaInteresse_" & SgIdioma & " as AreaInteresse " &_
						"FROM Relacionamento_Edicoes_AreaInteresse as REA " &_
						"Inner Join AreaInteresse as A ON A.ID_AreaInteresse = REA.ID_AreaInteresse " &_
						"WHERE " &_ 
						"	A.Ativo = 1 " &_
						"	AND REA.ID_Edicao = " & ID_Edicao & " " &_
						"ORDER BY AreaInteresse " 					
	Set RS_InteresseFeira = Server.CreateObject("ADODB.Recordset")
	RS_InteresseFeira.CursorType = 0
	RS_InteresseFeira.LockType = 1
	RS_InteresseFeira.Open SQL_InteresseFeira, Conexao
...
End If
	...
	SQL_Cargo = 		"SELECT " &_
						"	C.ID_Cargo as Id, " &_
						"	C.Cargo_" & SgIdioma & " as Cargo " &_
						"FROM Relacionamento_Edicoes_Cargo as REC " &_
						"INNER JOIN Cargo as C ON C.ID_Cargo = REC.ID_Cargo " &_
						"WHERE " &_
						"	C.Ativo = 1 " &_
						"	AND REC.ID_Edicao = " & ID_Edicao & " " &_
						"ORDER BY Cargo " 
...
<script language="javascript">
	...
	var msg_estado			= '<%=textos_array(48)(2)%>';
	var tp_formulario       = '<%=TP_Formulario%>';
	var aviso_doc_existe	= '<%=textos_array(59)(2)%>';
	var aviso_titulo_existe	= '<%=textos_array(60)(2)%>';
	var aviso_doc_empresa_titulo_existe	= '<%=textos_array(62)(2)%>';
	

	function troca() {
		var string = document.prcCadEmpresa.frmPriProdut.value;
		var string = string.replace(/([0-9])/g, "");
	document.prcCadEmpresa.frmPriProdut.value=string;
	}
</script>
</head>
...
<%
	' ABF 2013
	If ID_Edicao = "22" Then
%>	
	<table width="840" border="0" align="center" cellpadding="0" cellspacing="0">
	  <tr>
	    <td height="40" class="fs_13px arial cor_cinza2"><%=textos_array(56)(2)%></td>
	  </tr>
	</table>
<% 
	End If 
	...
	'ABF NE 2013
	If ID_Edicao = "35" Then
%>	
	<!--<table width="840" border="0" align="center" cellpadding="0" cellspacing="0">
	  <tr>
	    <td height="40" class="fs_13px arial cor_cinza2"><%=textos_array(63)(2)%></td>
	  </tr>
	</table>-->
<% 
	End If 
%>
    ...
    <div id="contForm">
    <!-- Form -->
	<form action="processar.asp" method="post" id="prcCadEmpresa" name="prcCadEmpresa" >
    ...
    <input type="hidden" id="produtos_inserir"	name="produtos_inserir" 	value="" >
    <input type="hidden" id="produtos_alterar"	name="produtos_alterar" 	value="" >
            <!-- Alert error -->
            ...
            <fieldset id="grupo1">
                ...
                <label style="width:390px"><!--Nome Fantasia--><%=textos_array(2)(2)%><input type="text" name="frmFantasia" id="frmFantasia" style="width:380px" max="150" maxlength="150"/></label>
                <label style="width:390px;"><!--Ramo de atividade-->
				
				<%
				
				if SgIdioma = "ENG" then
						Response.Write("Branch")
				else				
						Response.Write(textos_array(3)(2))
				end if				
				%>
                
                
                   <select name="frmRamo" id="frmRamo" style="width:390px">
	                   <option value="-" sigla="-" complemento='-' selected="selected"><!--Selecione-->-- <%=textos_array(36)(2)%> --</option>
						<%
                            ' Looping RamoAtividade
							OptionRamoOutros = ""
                            If not RS_RamoAtividade.BOF or not RS_RamoAtividade.EOF Then
                                While not RS_RamoAtividade.EOF
                                	If RS_RamoAtividade("Nome") = "Outros" or RS_RamoAtividade("Nome") = "Others" or RS_RamoAtividade("Nome") = "Other" or RS_RamoAtividade("Nome") = "Otros" Then
                                		OptionRamoOutros = "<option value='" & RS_RamoAtividade("Id") & "' complemento='True' sigla='" & caixaAltaBaixa("caixa_altabaixa",RS_RamoAtividade("Nome")) &"'>" & caixaAltaBaixa("caixa_altabaixa",RS_RamoAtividade("Nome")) &"</option>"
                                	Else
                                    %>
                                        <option value="<%=RS_RamoAtividade("Id")%>" complemento="<%=RS_RamoAtividade("Complemento")%>" sigla="<%=caixaAltaBaixa("caixa_altabaixa",RS_RamoAtividade("Nome"))%>"><%=caixaAltaBaixa("caixa_altabaixa",RS_RamoAtividade("Nome"))%></option>
                                    <%
    								End If
                                    RS_RamoAtividade.MoveNext()
                                Wend
                                RS_RamoAtividade.Close
                                response.write(OptionRamoOutros)
                            End if
                        %>
					</select>
                    <!--
					<input type="text" name="frmOptRamoOutros" id="frmOptRamoOutros" style="width:352px;display:none" max="150" maxlength="150"/><span class="bt_fechar" id="FecharRamoOutros" style="display:none" onclick="TipoRamoOff()">x</span>
                    -->
                </label>
                <label id="Complemento" style="width:390px"><!--Complemento--><%=textos_array(57)(2)%>
					<input type="text" name="frmOptRamoComplemento" id="frmOptRamoComplemento" style="width:352px;display:none" max="120" maxlength="120" onkeyup="this.value = this.value.toUpperCase();"/><span class="bt_fechar" id="FecharRamoComplemento" style="display:none" onclick="TipoRamoOff()">x</span>
                </label>
                <div id="grupo1a">
                    <label style="width:690px"><!--Ramais escolhidos--><span class="cor_preto">
                    
                    <% 
					
					if SgIdioma = "ENG" then
						Response.Write("Selected Branch")
					end if
					
					if SgIdioma = "PTB" then
						Response.Write("Ramos Selecionados Para")
					end if
					
					if SgIdioma = "ESP" then
						Response.Write("Ramos seleccionados")
					end if
					...
					%>
                     ...
                     <span id="nome_empresa" class="b"></span></span>
                        <div id="BoxListaRamos" class="div_parceria" style="display:table;">
                            <!-- Nenhum Ramo escolhido. -->
                        </div>
                    </label>
                </div>
			</fieldset>
			<fieldset id="grupo3">
				<label style="width:510px">
					<div style="width:110px" ><!--Principal produto--><%=textos_array(5)(2)%></div>
					<input type="text" name="frmPriProdut" id="frmPriProdut" style="width:300px" max="150" maxlength="150" onkeyup="troca(); this.value = this.value.toUpperCase();"/>
					<%
					' ========================================
					' Não exibir campos para internacional
					'If SgIdioma = "PTB" Then
					%>
	                    <span id="bt_busca_produto" class="bt_adicionar_produto">
                        
                        <% 
					
					if SgIdioma = "ENG" then
						Response.Write("Add Product")
					end if
					
					if SgIdioma = "PTB" then
						Response.Write("Adicionar Produto")
					end if
					
					if SgIdioma = "ESP" then
						Response.Write("Agregar Producto ")
					end if
					...
					%>
	                    ...
	                    <!-- <span id="bt_faq" class="bt_faq"></span> -->
                    <% 
                    'End If
					' ========================================
                    %>
			...
			<fieldset id="grupo2">
				<label style="width:690px"><!--Principal produto--><span class="cor_preto">
                
<% 
					
					if SgIdioma = "ENG" then
						Response.Write("Main Product List ")
					end if
					
					if SgIdioma = "PTB" then
						Response.Write("Lista de Produto Principal")
					end if
					...
					<div id="BoxListaProdutos" class="div_parceria" style="display:table;">
						<ul id="ListaProdutos">
						</ul>
						<div id="TitulosListaProdutosCadastrados"> <!-- Produtos Cadastrados Anteriormente --> </div>
						<ul id="ListaProdutosCadastrados">
						</ul>
					</div>
				</label>
			</fieldset>
            <fieldset id="grupo4">
                <label style="width:95px"><!--DDI--><%=textos_array(28)(2)%><input type="text" name="frmDDIEmpresa" id="frmDDIEmpresa" style="width:85px" max="3" maxlength="3"/></label>
                    <label style="width:95px"><!--DDD--><%=textos_array(29)(2)%><input type="text" name="frmDDDEmpresa" id="frmDDDEmpresa" style="width:85px" max="3" maxlength="3"/></label>
            ...
            <fieldset id="grupo5">
                <label style="width:310px">
					...
					If SgIdioma = "PTB" Then
					%>
                    <span  id="bt_busca_cpf" class="bt_busca" onclick="getEndereco()">Buscar CEP</span>
                    <% 
                    End If
					' ========================================
                    %>
            ...
            <fieldset id="grupo6">
                ...
                <%  
                	' Verificando qual campo aprensetar
                	If SgIdioma = "PTB" Then
                %>	    
				...
				<% Else %>
					<select name="frmEstado" id="frmEstado" style="width:250px">
                   		<option value="28" sigla="EX" selected><%=textos_array(48)(2)%></option>
					</select>
				<% End If %>
                </label>
                    	...
                    	<option value="-"><!--Selecione-->-- <%=textos_array(36)(2)%> --</option>
                            ...
                            If not RS_Pais.BOF or not RS_Pais.EOF Then
                                While not RS_Pais.EOF 
									selecionado = ""
									If RS_Pais("Pais") = "Brasil" and SgIdioma = "PTB" Then selecionado = " selected "
									%>
                ...
                <br/>
            	<label style="width:390px"><!--Site--><%=textos_array(15)(2)%><input type="text" name="frmSite" id="frmSite" style="width:380px" max="100" maxlength="100"/></label>
            	<br/>
            	<label style="width:260px"><!--Número de funcionários--><%=textos_array(14)(2)%>
                	<select name="frmQtdFunc" id="frmQtdFunc" style="width:250px;">
                        <option value="-" selected="selected"><!--Selecione-->-- <%=textos_array(36)(2)%> --</option>
                        	<%
								' Looping Funcionarios
								If not RS_Funcionarios.BOF or not RS_Funcionarios.EOF Then
									While not RS_Funcionarios.EOF
										%>
										   <option value="<%=RS_Funcionarios("Id") %>"><%=RS_Funcionarios("Func") %></option>
										<%
                ...
                <br/><!--O que você procura na
           	  	<label style="width:700px">
           	  			<% 	If ID_Edicao = 10 Then
           	  					procura = textos_array(55)(2)
           	  				else
           	  					procura = textos_array(16)(2) & " <strong> " & Evento & " </strong>?"
           	  				End If
           	  			%>
           	  			<%=procura%>
           	  			</label>
                	<div id="divInteresse">
                        <ul>
                            <%
                                ...
                                If not RS_InteresseFeira.BOF or not RS_InteresseFeira.EOF Then
                                    z = 0
									While not RS_InteresseFeira.EOF
									
										If RS_InteresseFeira("AreaInteresse") = "Outros" or RS_InteresseFeira("AreaInteresse") = "Others" or RS_InteresseFeira("AreaInteresse") = "Other" or RS_InteresseFeira("AreaInteresse") = "Otros" Then
                                			LiInteresseOutros = "<li><input type='checkbox' name='frmInteresse' id='frmInteresse' value='" & RS_InteresseFeira("Id") & "' /><span class='cursor' onclick=""$('input[name=frmInteresse]')[z].click();"">&nbsp;&nbsp;" & RS_InteresseFeira("AreaInteresse") & "</span></li>"
                                		Else
                                        %>
                                           <li><input type="checkbox" name="frmInteresse" id="frmInteresse" value="<%=RS_InteresseFeira("Id") %>"/><span class="cursor" onclick="$('input[name=frmInteresse]')[<%=z%>].click();">&nbsp;&nbsp;<%=RS_InteresseFeira("AreaInteresse") %></span></li>
                                           
                                        <%
											z = z + 1
										End If
                                        RS_InteresseFeira.MoveNext()
                                    Wend
									response.write(Replace(LiInteresseOutros, "z", z))
                                    RS_InteresseFeira.Close
                                End if
                            %>
                        </ul>       
					</div>
            ...
            <fieldset id="grupo8">
                ...
                <label style="width:450px">
                    ...
                    <input type="text" name="frmCPF" id="frmCPF" style="width:290px" max="14" maxlength="14"/><span class="bt_busca" onclick="getCadastroCPF()"><!--Buscar CPF--><%=textos_array(19)(2)%></span>
                </label>
      		</fieldset>
            <fieldset id="grupo9">
                ...
                <br/>
                <label style="width:95px"><!--DDI--><%=textos_array(28)(2)%><input type="text" name="frmDDI" id="frmDDI" style="width:85px" max="3" maxlength="3"/></label>
                <label style="width:95px"><!--DDD--><%=textos_array(29)(2)%><input type="text" name="frmDDD" id="frmDDD" style="width:85px" max="3" maxlength="3"/></label>
                ...
                <br/>
                <label style="width:95px"><!--DDI--><%=textos_array(28)(2)%><input type="text" name="frmDDI2" id="frmDDI2" style="width:85px" max="3" maxlength="3"/></label>
                <label style="width:95px"><!--DDD--><%=textos_array(29)(2)%><input type="text" name="frmDDD2" id="frmDDD2" style="width:85px" max="3" maxlength="3"/></label>
               ...
               <div id="RecebeSmS2">
                ...
                <br/>
                <!--label style="width:390px;">Código do Convite (Caso Possua):
                <br/>
                <input type="text" name="frmCodigoConvite" id="frmCodigoConvite" style="width:380px" max="150" maxlength="8"/></label-->
                <div id="divNewsletter" style="display:block; width:790px; margin-top:10px;">
                    <div id="radio3" style="width:790px;">
                        <div class="radiopos"><input type="checkbox" name="frmNewsletter" id="frmNewsletter" value="1" checked/>&nbsp;<%=textos_array(38)(2)%>&nbsp;</div>
                    </div>
                </div>
                <br/>
            </fieldset>
            ...
            <% 
	            ...
	            <% 								
								...
								If not RS_Opcoes.BOF or not RS_Opcoes.EOF Then
									...
									' Se o campo for do tipo text nao coloca valor no campo
									If RS_Perguntas("Tipo") = 1 Then
										ValorCampo = ""
									End If
	       		%>               
			...
			<%
			If Session("cliente_edicao") = 46 Then
			%>
            <div style="display: none; margin-top: 15px; float: left;" id="texto_final" class="fs_12px arial cor_cinza2">
            	<br/>Quer adquirir seu <strong>Ingresso</strong> antecipadamente? <strong>Clique</strong> no banner que estará na próxima página de <strong>Confirmação de Cadastro</strong><br/>
            </div>
            <%
            End If
			%>
